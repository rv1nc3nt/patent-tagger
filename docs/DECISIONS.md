...
## 2026-09-25 — View screen with find and highlights (user request)

**Question:** The user asked for a View tab to navigate a patent's content (metadata, abstract, description, claims, drawings), search its text, and highlight passages with comments.

**Decisions (SPEC 4.2 and 8 updated):**
- **Storage:** new table `annotations` (migration `003_annotations.sql`), managed by `core::annotations`. A highlight covers `start..end` of one section (`title`, `abstract`, `description`, `claims`) in UTF-16 code units, the unit of the webview's strings and selections, so the UI uses offsets as they are. Colours are a fixed set of four; the comment is optional.
- **Changed text:** the highlight's `quote` is stored. On listing, a highlight whose offsets no longer hold its quote moves to the nearest occurrence of the quote; if there is none it is `detached`: listed with its quote and comment, but not drawn. The new position is computed on each read, not written back.
- **Outline:** `core::outline` splits the stored description into headings and paragraphs, and the claims into claims with their references (`claim 1`, `claims 1 or 2`, `claims 2 to 4`, `claims 1-3`, `any of the foregoing claims`). An unnumbered description line is a heading when it has at most 120 characters, starts with a capital letter and does not end with `.`, `,` or `;`. The capital-letter rule was added after EP1000000's lowercase figure list (`figure 5 is … ; and`) showed up as headings. Only references to earlier claims count as dependencies; forward references are still links.
- **Find** runs in the frontend over the text the page already shows, like a browser's own find: it is presentation, not business logic. It needs at least 2 characters and is case-insensitive. It covers the open document only; Library search is unchanged.
