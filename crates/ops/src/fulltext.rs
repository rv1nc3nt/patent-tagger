...
pub fn parse_description(xml: &str) -> Result<Option<(String, String)>, OpsError> {
    let doc = Document::parse(xml).map_err(|e| OpsError::Parse(format!("invalid XML: {e}")))?;
    let Some(description) = doc.descendants().find(|n| n.has_tag_name("description")) else {
        return Ok(None);
    };
...
/// one `<claim>`.
pub fn parse_claims(xml: &str) -> Result<Option<(String, String)>, OpsError> {
    let doc = Document::parse(xml).map_err(|e| OpsError::Parse(format!("invalid XML: {e}")))?;
    let Some(claims) = doc.descendants().find(|n| n.has_tag_name("claims")) else {
        return Ok(None);
    };
...
mod tests {
    ...
    fn description_keeps_paragraph_numbers_and_normalises_whitespace() {
        let (lang, text) = parse_description(&fixture("ep1000000_a1_description.xml")).unwrap().unwrap();
        assert_eq!(lang, "EN");
    ...
    fn claims_keeps_claim_numbers_one_per_line() {
        let (lang, text) = parse_claims(&fixture("ep1000000_a1_claims.xml")).unwrap().unwrap();
        assert_eq!(lang, "EN");
    ...
    #[test]
    fn b1_claims_are_genuinely_trilingual() {
        ...
        // Parsing should pick out only the English one, matching SPEC
        // 5.5's "for an EP B1, take the English claims" rule (the
        // selection itself is core_lib's job; this just confirms parsing
        // targets a specific lang element rather than mixing all three).
        let english_claims = doc
            .descendants()
            .find(|n| n.has_tag_name("claims") && n.attribute("lang") == Some("EN"))
            .unwrap();
        assert!(english_claims.descendants().any(|n| n.has_tag_name("claim-text")));
    }

    #[test]
    fn german_description_parses_the_same_way_as_english() {
        let (lang, text) =
            parse_description(&fixture("synthetic_ep_fr_de_only_description.xml")).unwrap().unwrap();
        assert_eq!(lang, "DE");
